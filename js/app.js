...
async function startMedia() {
    if (!localStream) {
        try {
            ...
            // Começa com cam/mic desativados no botão, mas capturados
...
function addStream(video, stream, id) {
    ...
    video.muted = false; // Áudio do colega DEVE ser ouvido
    video.autoplay = true;
    video.playsinline = true;
    video.style.width = "100%";
    video.style.border = "1px solid var(--hot-pink)";
    videoGrid.append(video);
    video.play().catch(() => console.log("Usuário precisa interagir para som tocar."));
}
