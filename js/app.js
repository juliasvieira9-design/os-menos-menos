...
// 1. GERAR ID ALEATÓRIO E CONECTAR PEERJS
...
// Mostra o ID na tela quando o PeerJS abrir
peer.on('open', (id) => {
    if (myIdDisplay) {
...
// 2. RECEBER CHAMADA (Quando seu amigo te liga)
...
// 3. LIGAR PARA O AMIGO (Botão Conectar)
document.getElementById('btn-connect').onclick = async () => {
    const remoteId = document.getElementById('peer-id-input').value.trim().toUpperCase();
    if (!remoteId || remoteId === randomId) return alert("Digite um ID válido!");
...
// 4. ADICIONAR VÍDEO DO AMIGO COM ÁUDIO ATIVO
function addStream(video, stream, id) {
    ...
    video.muted = false; // Áudio do amigo ON
    video.volume = 1.0;
